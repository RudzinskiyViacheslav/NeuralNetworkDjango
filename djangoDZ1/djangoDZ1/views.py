...
imageClassList = {'0': 'Компуктер', '1': 'ПОПУгай', '2': 'Вамбата'}
...
def predictImage(request):
    ...
    context = {'scorePrediction': scorePrediction, 'image': filePathName}
    return render(request, 'scorepage.html', context)

def predictImageData(modelName, filePath):
    img = Image.open(filePath).convert("RGB")
    img = np.asarray(img.resize((32, 32), Image.ANTIALIAS))
    sess = onnxruntime.InferenceSession(r'C:\Users\vrudzinskiy\Desktop\dz1\djangoDZ1\media\models\dz1_RESNET20_rudz.onnx')
